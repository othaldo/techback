export const exerciseLibrary = {
  "Passive Bauchlage / Sphinx": {
    description:
      "Lege dich flach auf den Bauch. Die Beine liegen gestreckt, die Arme winkelst du an und stützt dich auf die Unterarme - die Ellbogen direkt unter den Schultern. Der Oberkörper ist leicht aufgerichtet. Atme ruhig und bleibe in dieser Position. Wenn möglich, kannst du dich weiter aufrichten wie in der Sphinx-Stellung (Brust raus, Schultern tief).",
    image: process.env.PUBLIC_URL + "/exercises/sphinx.jpg",
    category: "Entlastung",
    credit: {
      label: "Pexels - A frame in motion",
      url: "https://www.pexels.com/photo/a-woman-meditating-in-the-garden-11387441/",
    },
  },
  "Glute Bridge": {
    description:
      "Lege dich auf den Rücken, stelle die Füße hüftbreit auf den Boden, die Arme liegen neben dem Körper. Hebe dein Becken langsam nach oben, bis Knie, Hüfte und Schultern eine Linie bilden. Halte kurz und senke es dann langsam wieder ab. Gesäß aktiv anspannen, nicht ins Hohlkreuz fallen.",
    video: process.env.PUBLIC_URL + "/exercises/glute-bridge.mp4",
    category: "Aktivierung",
    credit: {
      label: "Pexels - Woman Doing Glute Bridge Exercise",
      url: "https://www.pexels.com/video/woman-doing-glute-bridge-exercise-6525487/",
    },
  },
  "Bird-Dog": {
    description:
      "Gehe in den Vierfüßlerstand (Hände unter Schultern, Knie unter Hüften). Strecke langsam den rechten Arm nach vorn und das linke Bein nach hinten - beide parallel zum Boden. Halte die Position ruhig für einige Sekunden, dann wechsle die Seite. Achte auf einen stabilen Rücken - kein Durchhängen.",
    category: "Stabilität",
  },
  "Hüftbeuger-Dehnung": {
    description:
      "Gehe in einen tiefen Ausfallschritt mit einem Bein vorn und dem anderen nach hinten gestreckt. Das hintere Knie liegt auf dem Boden. Verlagere dein Gewicht leicht nach vorn, bis du eine Dehnung in der Leiste spürst. Oberkörper bleibt aufrecht. Halte diese Position 30-60 Sekunden pro Seite.",
    category: "Dehnung",
  },
  "Cat-Cow": {
    description:
      "Gehe in den Vierfüßlerstand. Beim Einatmen hebst du den Kopf, ziehst die Schultern zurück und kippst das Becken leicht - der Rücken wird hohl (Cow). Beim Ausatmen machst du einen Katzenbuckel (Cat), ziehst das Kinn zur Brust und das Becken nach vorn. Langsam und gleichmäßig im Wechsel.",
    category: "Mobilisierung",
  },
  Hängen: {
    description:
      "Hänge dich an eine Klimmzugstange oder Türreck, die Hände schulterbreit. Lasse dein Körpergewicht locker hängen - nicht ziehen, einfach entspannen. Schultern nicht nach oben ziehen. Wenn es zu viel wird, absetzen. 15-30 Sekunden reichen aus.",
    category: "Entlastung",
  },
  "Piriformis-Dehnung": {
    description:
      "Setze dich auf den Boden oder lege dich auf den Rücken. Lege den rechten Fuß über das linke Knie. Ziehe das linke Bein vorsichtig Richtung Brust, bis du eine Dehnung im rechten Gesäß spürst. Halte die Position ruhig, dann Seite wechseln. Nicht überdehnen.",
    category: "Dehnung",
  },
  "Kurzer Spaziergang 🚶‍♂️": {
    description:
      "Ein lockerer Spaziergang von 10-15 Minuten. Kein Sport, kein Tempo - einfach entspannt gehen, tief durchatmen und Bewegung genießen. Fördert die Durchblutung und hilft dem Rücken, sich zu entspannen.",
    category: "Bewegung",
  },
  "Herabschauender Hund": {
    description:
      "Gehe in den Vierfüßlerstand. Hebe das Becken langsam nach oben, bis du ein umgekehrtes V mit deinem Körper bildest. Die Arme und Beine sind gestreckt, die Fersen Richtung Boden. Der Rücken ist gerade, der Kopf entspannt. Halte die Position 30-60 Sekunden.",
    category: "Mobilisierung",
  },
  Plank: {
    description:
      "Stütze dich auf Unterarme und Zehen, Körper bildet eine gerade Linie. Bauch, Rücken und Gesäß anspannen. Halte die Position ruhig für eine bestimmte Zeit, ohne durchzuhängen.",
    category: "Stabilität",
  },
  "Side Plank": {
    description:
      "Stütze dich seitlich auf Unterarm und Fuß, der Körper bildet eine Linie. Halte die Position ruhig und kontrolliert, ohne abzusinken. Danach Seite wechseln.",
    category: "Stabilität",
  },
  "Cobra / Mini-Cobra": {
    description:
      "Lege dich auf den Bauch, Hände unter die Schultern. Hebe langsam den Oberkörper, während das Becken am Boden bleibt. Halte die Position nur leicht angehoben (Mini-Cobra) oder weiter gestreckt (Cobra), ohne Schmerzen.",
    category: "Mobilisierung",
  },
};
